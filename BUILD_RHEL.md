...
[Bake file (`docker-bake-rhel.hcl`)](docker-bake-rhel.hcl).
...
```bash
docker buildx bake -f -f docker-bake-rhel.hcl --check
```
...
```bash
docker buildx bake -f -f docker-bake-rhel.hcl --push
```
...
```bash
docker buildx bake -f -f docker-bake-rhel.hcl --push postgresql-17-minimal-trixie
```

You can also limit the build to a single platform, for example AMD64, with:

```bash
docker buildx bake -f -f docker-bake-rhel.hcl --push --set "*.platform=linux/amd64"
```
...
`postgreSQLPreviewVersions` array in the `docker-bake-rhel.hcl` file. For example:
...
```bash
docker buildx bake -f -f docker-bake-rhel.hcl --print
```
