...
target "default" {
  ...
  annotations = [
    ...
    "index,manifest:org.opencontainers.image.base.name=docker.io/library/oraclelinux:${tag(base)}"
#    ,"index,manifest:org.opencontainers.image.base.digest=${digest(base)}"
  ...
  labels = {
    ...
    "org.opencontainers.image.base.name" = "docker.io/library/oraclelinux:${tag(base)}"
...
function distroVersion {
  params = [ imageNameWithSha ]
  result = format("ol%s", replace(tag(imageNameWithSha), "-slim", ""))
}
...
function getExtensionsString {
    ...
    result = (isPreview(version) == true) ? "" : join(" ", formatlist("%s_%s", extensions, getMajor(version)))
}
